...
<div class="well well-lg wrap-msg">
	<div class ="row">
		...
		if(!isset($_GET["sort"])) {
		?>
		<div class="col-sm-2"><a href ="city.php" class="btn btn-success" style ="width: 80%; margin-top: 18px;"><span class="fa fa-thumbs-up"></span> Sorted by Likes</a></div>
		...
		?>
		<div class="col-sm-2"><a href ="city.php" class="btn btn-primary" style ="width: 80%; margin-top: 18px;"><span class="fa fa-thumbs-up"></span> Sort by Likes</a></div>
...
<?php 
	if(!isset($_GET["sort"])) {
	$sql = "call get_cities_sorted_by_likes()";
	$stmt = $mysqli->prepare($sql);
	if($stmt)
	{
		$stmt->execute();
...
$cname = $_GET["cname"];
$sql = "call view_cities_according_to_rating_criteria(?)";
	$stmt = $mysqli->prepare($sql);
