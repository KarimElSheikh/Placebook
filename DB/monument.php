...
<div class="well well-lg wrap-msg">
	<div class ="row">
		...
		<div class="col-sm-2"><a href ="monument.php" class="btn btn-primary" style ="width: 80%; margin-top: 18px;"><span class="fa fa-thumbs-up"></span> Likes</a></div>
		...
		<ul class="dropdown-menu" role="menu">
			<?php
				...
				if($stmt)
				{
					...
					while ($stmt->fetch()) { ?>
			<li><a href="monument.php?sort=2&cname=<?php echo $cname;?>"><?php echo $cname;?></a></li>
			<?php }} $stmt->close(); ?>
		</ul>
		</div>
	</div>
...
<?php 
	if(!isset($_GET["sort"])) {
	$sql = "call get_monument_with_most_likes()";
	$stmt = $mysqli->prepare($sql);
	if($stmt)
	{
		$stmt->execute();
