...
<div class="well well-lg wrap-msg" style="width: 1300px;">
	<div class ="row">
		...
		if(!isset($_GET["sort"])) {
		?>
		<div class="col-sm-2"><a href ="restaurant.php" class="btn btn-success" style ="width: 80%; margin-top: 18px;"><span class="fa fa-thumbs-up"></span> Sorted by Likes</a></div>
		...
		?>
		<div class="col-sm-2"><a href ="restaurant.php" class="btn btn-primary" style ="width: 80%; margin-top: 18px;"><span class="fa fa-thumbs-up"></span> Sort by Likes</a></div>
		...
		<ul class="dropdown-menu" role="menu">
			<?php
				...
				if($stmt)
				{
					...
					while ($stmt->fetch()) { ?>
			<li><a href="restaurant.php?sort=2&cname=<?php echo $cname;?>"><?php echo utf8_encode($cname);?></a></li>
			<?php }} $stmt->close(); ?>
		</ul>
		</div>
	</div>
...
<?php 
	if(!isset($_GET["sort"])) {
	$sql = "call get_restaurants_sorted_by_likes()";
	$stmt = $mysqli->prepare($sql);
	if($stmt)
	{
		$stmt->execute();
